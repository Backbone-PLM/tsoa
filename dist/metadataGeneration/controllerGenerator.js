...
var ControllerGenerator = /** @class */ (function () {
    ...
    ControllerGenerator.prototype.getInheritanceList = function () {
        if (!this.node.heritageClauses || !this.node.heritageClauses.length) {
            return [];
        }
        return this.node.heritageClauses
            .reduce(function (acc, node) { return acc.concat(node.types); }, [])
            .map(function (nodeType) { return nodeType.expression.escapedText; });
    };
